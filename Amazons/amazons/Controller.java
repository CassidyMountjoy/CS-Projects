...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("manual ((?i:black)|(?i:white))", this:: manual),
        new Command("auto ((?i:black)|(?i:white))", this:: auto),
        new Command("turn", this:: turn)
    };
    ...
    private void doNew(Matcher mat) {
        _board.init();
        _winner = null;
    }
    ...
    private void doDump(Matcher unused) {
        System.out.printf("===%n%s===%n", _board.toString());
    }

    /** Does the move.
     * @param used the string.
     * */
    private void doMove(Matcher used) {
        _board.makeMove(Move.mv(used.group(0)));
        Iterator<Move> iter1 = board().legalMoves(board().turn());
        if (!iter1.hasNext()) {
            if (board().turn() == WHITE) {
                _winner = BLACK;
     ...
     * @param used creates player.
     * */
    private void turn(Matcher used) {
        System.out.println(_board.turn());
    }

    /** Creates a manual player.
     * @param used andothe player.
     * */
    private void manual(Matcher used) {
        _initialized = true;
        Piece player = WHITE;
        if (used.group(1).charAt(0) == 'w' || used.group(1).charAt(0) == 'W') {
            player = WHITE;
            _white = _manualPlayerTemplate.create(player, this);
        }
        if (used.group(1).charAt(0) == 'B' || used.group(1).charAt(0) == 'b') {
            player = BLACK;
            _black = _manualPlayerTemplate.create(player, this);
        }
    }

    /** Creates a AI player.
     * @param used automatic.
     * */
    private void auto(Matcher used) {
        _initialized = true;
        Piece player = WHITE;
        if (used.group(1).equals("WHITE") || used.group(1).equals("white")) {
            player = WHITE;
            _white = _autoPlayerTemplate.create(player, this);
        }
        if (used.group(1).equals("BLACK") || used.group(1).equals("black")) {
            player = BLACK;
            _black = _autoPlayerTemplate.create(player, this);
        }
    }
