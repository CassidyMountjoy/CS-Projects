...
class Model {
     ...
     * This is for centers.
     */
    private ArrayList listcenter = new ArrayList();
    /**
    ...
    void copy(Model model) {
        if (model == this) {
            ...
            ArrayList interC = new ArrayList();
            for (int i = 0; i < model.listcenter.size(); i++) {
                interC.add(model.listcenter.get(i));
            }
            this.listcenter = interC;
            ArrayList interB = new ArrayList();
            for (int i = 0; i < model.boundaries.size(); i++) {
                interB.add(model.boundaries.get(i));
            }
            this.boundaries = interB;
            this.board = new int[model.xlim()][model.ylim()];
            this.columns = model.columns;
            this.rows1 = model.rows1;
        }
    }
    ...
    void init(int cols, int rows) {
        ...
        for (int i = 1; i < xlim - 1; i += 2) {
            boundaries.add(pl(i, 0));
            boundaries.add(pl(i, ylim - 1));
        }
        for (int j = 1; j < ylim - 1; j += 2) {
            boundaries.add(pl(0, j));
            boundaries.add(pl(xlim - 1, j));
        }
    }
        ...
        for (Place cell : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (isBoundary(boundary) && region.contains(nextCell)
                        || center != nextCell && isCenter(nextCell)
    ...
    HashSet<Place> findGalaxy(Place center) {
        ...
        List<Place> cells = containing(center);

        for (Place i : cells) {
            accreteRegion(i, galaxy);
        }
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        markAll(region, 1);
        ArrayList<Place> aregion = new ArrayList<>(region);
        for (int i = 0; i < board.length; i++) {
            if (unmarkedSymAdjacent(center, aregion).isEmpty()) {
                break;
            }
            aregion.addAll(unmarkedSymAdjacent(center, aregion));
        }
        region.addAll(aregion);
        markAll(region, 0);
        return region;
    }
    ...
    void placeCenter(Place p) {
        if (!isCenter(p.x, p.y) && !isBoundary(pl(p.x, p.y))) {
            listcenter.add(pl(p.x, p.y));
        }
    }
    ...
    Place opposing(Place p0, Place p) {
        int xchange = p0.x - p.x;
        int ychange = p0.y - p.y;
        if (isCell(p0.x + xchange, p0.y + ychange)) {
            Place newpoint = pl(p0.x + xchange, p0.y + ychange);
            return newpoint;
        } else {
            return null;
        }
     ...
     * "containing" PLACE if all of the cells are
     ...
     */
    List<Place> centers() {
        return Collections.unmodifiableList(listcenter);
    }
    ...
    /**
     * Returns all cells containing center.
     * @param place dot on cell.
     */
    List<Place> containing(Place place) {
        if (isCell(place)) {
            return asList(place);
        } else if (isVert(place)) {
            return asList(place.move(-1, 0),
